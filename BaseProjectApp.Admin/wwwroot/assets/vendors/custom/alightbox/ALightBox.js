(function($) {
	$.fn.ALightBox = function(options) {
        ...
        var imageSelector = ".alb_item .innerpolaroid .imgCover";
		var index = 0;
		...
		for(var i = 0; i < totalItems; i++) {
            ...
            var tag = $(items[i])[0].firstElementChild.firstElementChild.firstElementChild.localName;

			/*-- Is the tag an a-tag? --*/
			if(tag == 'a' && settings.showYoutubeThumbnails) {
                var videoID = $(items[i])[0].firstElementChild.firstElementChild.firstElementChild.getAttribute("href").match(/^.*(youtu.be\/|v\/|u\/\w\/|embed\/|watch\?v=|\&v=)([^#\&\?]*).*/)[2];

                $(items[i])[0].firstElementChild.firstElementChild.firstElementChild.innerHTML = `<img src="https://i.ytimg.com/vi/${videoID}/maxresdefault.jpg"/>`;
		...
		function Update() {
            LoadContent($(items[index].firstElementChild.firstElementChild.firstElementChild));
        ...
        $(this).find(imageSelector).stop().click(function (e) {
			e.preventDefault();
            Open($(this).parent().parent());
		});
